 ...
 */
class Ccff_Admin_ActDeact extends Ccff_Admin_Base {
	...
	public function initialize() {
		...
		register_activation_hook( CCFF_TEXTDOMAIN . '/' . CCFF_TEXTDOMAIN . '.php', array( __CLASS__, 'activate' ) );
		register_deactivation_hook( CCFF_TEXTDOMAIN . '/' . CCFF_TEXTDOMAIN . '.php', array( __CLASS__, 'deactivate' ) );
	...
	private static function single_activate() {
		...
		new Plugin_Requirements(
             CCFF_NAME, CCFF_TEXTDOMAIN, array(
	...
	public static function upgrade_procedure() {
		if ( is_admin() ) {
			$version = get_option( 'customer-chat-for-facebook-version' );
			if ( version_compare( CCFF_VERSION, $version, '>' ) ) {
				update_option( 'customer-chat-for-facebook-version', CCFF_VERSION );
				delete_option( CCFF_TEXTDOMAIN . '_fake-meta' );
			}
		}
	}
